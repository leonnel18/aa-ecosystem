"""
crm_auth.py — Zoho CRM OAuth2 Token Manager (reminder-job variant)

Copied from functions/monitor-job/crm_auth.py verbatim (not imported — matches
the existing no-cross-import pattern between Catalyst functions in this repo).
Same refresh-token exchange as functions/pipeline-job/crm_auth.py, but caches
the access token in a MODULE-LEVEL IN-MEMORY variable instead of a file, since
a file-based cache may not persist reliably across a Catalyst function's
