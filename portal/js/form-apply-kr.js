// form-apply-kr.js — KR (South Korea) Foundational Pre-Application Form logic
...
// Training-type slug → display name (Foundational only for KR)
const TYPE_LABELS = {
  ...
  Foundational: [
    {
      ...
      labelFil: "캠페인 경험을 어떻게 평가하시겠습니까? \"캠페인\"이란 사회적, 환경적 또는 정치적 목표를 달성하기 위한 조직적인 행동 과정을 의미합니다.",
    },
    {
      ...
      labelFil: "캠페인을 위한 전략 수립 및 전술 선택에 자신감을 느낍니다.",
    },
    {
      ...
      labelFil: "캠페인을 위해 이야기를 활용해 다른 사람들과 소통하거나 커뮤니케이션 전략을 수립하는 데 자신감을 느낍니다.",
    },
    {
      ...
      labelFil: "캠페인을 위한 토론이나 지식 공유 활동을 진행하는 데 자신감을 느낍니다.",
    },
    {
      ...
      labelFil: "캠페인을 지원하기 위해 다른 사람들과 네트워크를 구축하는 데 자신감을 느낍니다.",
    },
  ],
};
...
function wireCityProvinceMode(country) {
  const isKR = country === "South Korea" || country === "";
  ...
  if (isKR) {
    ...
    if (pTrig) { pTrig.textContent = "— 시/도 선택 —"; pTrig.classList.add("placeholder"); }
    const cInput = document.getElementById("city-search-input");
    if (cInput) { cInput.value = ""; cInput.disabled = true; cInput.placeholder = "먼저 시/도를 선택하세요…"; }
...
function wireProvinceDropdown() {
  const provinces = Object.keys(KR_PROVINCES).sort();
  ...
  function buildCityList(province) {
    currentMunis = (KR_PROVINCES[province] || []).slice().sort();
    cityInput.value = "";
    cityHidden.value = "";
    cityInput.disabled = false;
    cityInput.placeholder = "구/시 검색…";
  ...
  function renderCityOptions(q) {
    ...
    if (matches.length === 0 && q.length > 0) {
      cityList.innerHTML = `
        <div class="org-dropdown-item" style="color:var(--meta);cursor:default;font-size:13px">결과 없음 — 직접 입력 가능</div>
        <div class="org-add-row" id="city-enter-manual">"${escHtml(q)}" 사용하기</div>`;
  ...
  function renderChips() {
    ...
    if (checked.length === 0) {
      trigger.insertAdjacentHTML("afterbegin",
        `<span class="multiselect-placeholder">Select all that apply / 해당하는 모두 선택</span>`
      );
    } else {
...
async function init() {
  ...
  // Phone: strip non-digits, cap at 11, format as 01X XXXX XXXX
...
function populateBanner(t) {
  ...
  document.getElementById("banner-venue").textContent =
    t.Venue_Address || t.Organised_By || "South Korea";
...
function fmtDate(iso) {
  return new Date(iso).toLocaleDateString("ko-KR", { day: "numeric", month: "short", year: "numeric" });
}
...
function validateSection(sectionId) {
  ...
  if (sectionId === "sec-professional") {
    ...
    if (!orgInput) {
      orgErrEl.textContent = "이 항목은 필수입니다.";
      orgErrEl.style.display = "block";
      ok = false;
    } else if (!selectedOrgId && !addingNewOrg) {
      orgErrEl.textContent = "목록에서 단체를 선택하거나 '+ Add new organization'을 클릭해 주세요.";
      orgErrEl.style.display = "block";
      ok = false;
    } else {
...
function validatePhone(id) {
  ...
  // KR: 10-11 digits starting with 01
  const pass = digits.length >= 10 && digits.length <= 11 && digits.startsWith("01");
  field.classList.toggle("has-error", !pass);
  return pass;
}
...
function isKRCountry() {
  const v = (document.getElementById("Country_of_Residence")?.value ?? "South Korea");
  return v === "South Korea" || v === "";
}

function validateCityTown() {
  const errEl = document.getElementById("city-error");
  const val = isKRCountry()
...
function renderRatings() {
  ...
  container.innerHTML = config.map(item => {
    ...
    <div class="rating-item" id="rating-item-${rk}">
      ...
      <div class="field-error" style="display:none">이 항목을 평가해 주세요.</div>
    </div>`;
  }).join("");
}
...
function buildCustomQuestionHtml(q, i) {
  ...
  return `
    <div class="field" data-cq-idx="${i}">
      ...
      ${inputHtml}
      ${req ? `<div class="field-error">이 항목은 필수입니다.</div>` : ""}
    </div>
  `;
}
...
function buildPayload() {
  ...
  const data = {
    ...
    Email:                  val("Email"),
    Mobile: "+82" + (document.getElementById("Mobile")?.value ?? "").replace(/\D/g, ""),
    Address:                val("Address"),
    City_Province: isKRCountry()
