// form-apply-id.js — ID (Indonesia) Foundational Pre-Application Form logic
...
// Training-type slug → display name (Foundational only for ID)
const TYPE_LABELS = {
  ...
  Foundational: [
    {
      ...
      labelFil: "Bagaimana Anda menilai pengalaman Anda dalam berkampanye? Yang dimaksud \"kampanye\" adalah serangkaian tindakan terorganisir untuk mencapai tujuan sosial, lingkungan, atau politik.",
    },
    {
      ...
      labelFil: "Saya merasa percaya diri dalam membangun strategi dan memilih taktik untuk sebuah kampanye.",
    },
    {
      ...
      labelFil: "Saya merasa percaya diri menggunakan cerita untuk terhubung dengan orang lain dalam kampanye ATAU memulai strategi komunikasi untuk sebuah kampanye.",
    },
    {
      ...
      labelFil: "Saya merasa percaya diri dalam memfasilitasi diskusi atau kegiatan berbagi pengetahuan untuk kampanye.",
    },
    {
      ...
      labelFil: "Saya merasa percaya diri dalam membangun koneksi dengan orang lain untuk mendukung kampanye.",
    },
  ],
};
...
function wireCityProvinceMode(country) {
  const isID = country === "Indonesia" || country === "";
  ...
  if (isID) {
    ...
    if (cInput) { cInput.value = ""; cInput.disabled = true; cInput.placeholder = "Pilih provinsi dulu…"; }
...
function wireProvinceDropdown() {
  const provinces = Object.keys(ID_PROVINCES).sort();
  ...
  function buildCityList(province) {
    currentMunis = (ID_PROVINCES[province] || []).slice().sort();
    cityInput.value = "";
    cityHidden.value = "";
    cityInput.disabled = false;
    cityInput.placeholder = "Cari kota / kabupaten…";
  ...
  function renderCityOptions(q) {
    ...
    if (matches.length === 0 && q.length > 0) {
      cityList.innerHTML = `
        <div class="org-dropdown-item" style="color:var(--meta);cursor:default;font-size:13px">Tidak ada hasil — ketik untuk isi manual</div>
        <div class="org-add-row" id="city-enter-manual">Gunakan "${escHtml(q)}" sebagai kota</div>`;
  ...
  function renderChips() {
    ...
    if (checked.length === 0) {
      trigger.insertAdjacentHTML("afterbegin",
        `<span class="multiselect-placeholder">Select all that apply / Pilih semua yang sesuai</span>`
      );
    } else {
...
async function init() {
  ...
  // Phone: strip non-digits, cap at 12, format as 08XX XXXX XXXX
  document.getElementById("Mobile").addEventListener("input", function() {
    const digits = this.value.replace(/\D/g, "").slice(0, 12);
    let fmt = digits;
    if (digits.length > 8) fmt = digits.slice(0,4) + " " + digits.slice(4,8) + " " + digits.slice(8);
...
function populateBanner(t) {
  ...
  document.getElementById("banner-venue").textContent =
    t.Venue_Address || t.Organised_By || "Indonesia";
...
function validateSection(sectionId) {
  ...
  if (sectionId === "sec-professional") {
    ...
    if (!orgInput) {
      orgErrEl.textContent = "Kolom ini wajib diisi.";
      orgErrEl.style.display = "block";
      ok = false;
    } else if (!selectedOrgId && !addingNewOrg) {
      orgErrEl.textContent = "Pilih organisasi dari daftar atau klik '+ Add new organization'.";
      orgErrEl.style.display = "block";
      ok = false;
    } else {
...
function validatePhone(id) {
  ...
  // ID: 9-12 digits starting with 08
  const pass = digits.length >= 9 && digits.length <= 12 && digits.startsWith("08");
  field.classList.toggle("has-error", !pass);
  return pass;
}
...
function isIDCountry() {
  const v = (document.getElementById("Country_of_Residence")?.value ?? "Indonesia");
  return v === "Indonesia" || v === "";
}

function validateCityTown() {
  const errEl = document.getElementById("city-error");
  const val = isIDCountry()
...
function buildCustomQuestionHtml(q, i) {
  ...
  return `
    <div class="field" data-cq-idx="${i}">
      ...
      ${inputHtml}
      ${req ? `<div class="field-error">Kolom ini wajib diisi.</div>` : ""}
    </div>
  `;
}
...
function buildPayload() {
  ...
  const data = {
    ...
    Email:                  val("Email"),
    Mobile: "+62" + (document.getElementById("Mobile")?.value ?? "").replace(/\D/g, ""),
    Address:                val("Address"),
    City_Province: isIDCountry()
