...
// ── State ──────────────────────────────────────────────────────────────────
let answeredList   = [];  // [{ name, q1, q2, q3, q4, q5, q6 }]
...
function parseResponses(raw) {
  const result = { q1: "", q2: "", q3: "", q4: "", q5: "", q6: "" };
...
function renderAnsweredTable() {
  ...
  answeredList.forEach((p, i) => {
    ...
    tr.innerHTML = `
      ...
      ${Q_PROPS.map((q) => `<td class="answer">${renderAnswerCell(p[q])}</td>`).join("")}
    `;
...
btnDownload.addEventListener("click", () => {
  ...
  const answeredRows = [
    ["Name", "Rating Keseluruhan", "Rating Sesi Pleno", "Rating Sesi Kelompok", "Menikmati Sesi", "Saran Perbaikan", "Masukan Lainnya"],
    ...answeredList.map((p) => [p.name, p.q1, p.q2, p.q3, p.q4, p.q5, p.q6]),
  ];
